...
| 필드 | 용도 |
|------|------|
| `파일` | 실제 소스 코드 대조 (보고서 내용 검증) |
| `라인` | 실제 소스 코드 대조 (보고서 내용 검증) |
| `규칙` | SonarQube 규칙 기준으로 검증 |
| `메시지` | 문제 요약과 일치하는지 확인 |
...
> **주의**: 소스 코드 파일(`issue_data.파일`)도 직접 Read/Grep/Glob으로 열어서 보고서 내용과 대조해야 합니다.
...
|-------------|----------|--------|
| `"analysis"` | `reports/{issue_key}/02_analysis_review.md` | sonar-analyze (결과 확인) |
| `"fix"` | `reports/{issue_key}/05_fix_review.md` | sonar-develop (결과 확인) |

### 스프레드시트 변경

없음. 상태 업데이트는 **호출 스킬**(sonar-analyze 또는 sonar-develop)이 수행합니다.
